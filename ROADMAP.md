...
- 🔴 **Aberto — encontrado na RFC-029 (v1.48.0)** — App inteiro inacessível por padrão: `SUPABASE_URL` vazio em `js/supabase-config.js` combinado com o gate de conta obrigatória (RFC-027) trava qualquer sessão nova atrás de uma tela terminal, sem ação possível além de fechar a aba. Gravidade: **crítica**, mas fora do escopo da RFC-029 (herdado da RFC-027, commit `a534bdb`, já em `main` antes desta sessão) — reportado pelo QA Engineer porque foi o que impediu o teste do fluxo de navegação real (clique na aba Aprender) daquela RFC.
  - **Onde**: `js/supabase-config.js` (linha 26-27) tem `SUPABASE_URL = ""` vazio, enquanto `SUPABASE_ANON_KEY` já vem preenchida com uma chave real. `Cloud.isAvailable()` retorna `false` (exige URL e chave truthy) → `App.ensureCloudAvailable()` (`js/app.js`, linha ~113) exibe `#cloudUnavailableScreen` — o "Gate 0" já documentado no próprio `index.html` (linha 59-61) como tela terminal. Confirmado ao vivo via CDP: `trailContainer` nunca chega a renderizar em uma sessão sem `localStorage` prévio.
  - **Contradição de documentação encontrada junto**: o comentário do próprio `js/supabase-config.js` (linhas 21-23) ainda promete que, com os dois valores em branco, "o PolvIn continua funcionando 100% normalmente no modo local (localStorage)" — isso deixou de ser verdade desde a RFC-027 (conta virou obrigatória), mas o comentário não foi atualizado. Mesmo problema no `README.md` (linha ~240), que ainda descreve o arquivo como "credenciais do Supabase (opcional, em branco por padrão)".
  - **Decisão pendente antes de corrigir**: (a) se o correto é preencher `SUPABASE_URL` de verdade antes de qualquer commit em `main` (aí o problema é só configuração de ambiente, não código), ou (b) se `js/supabase-config.js` deve continuar vazio no repositório por design (nunca commitar credenciais reais) e o Gate 0 está certo em bloquear — caso em que os comentários do próprio arquivo e do `README.md` precisam ser corrigidos para não prometer um modo local que não existe mais. De qualquer forma, os comentários enganosos já deveriam ser corrigidos independente da decisão (a) vs. (b); a correção completa (código + doc) merece uma RFC própria.
  - **Donos sugeridos**: Backend Engineer/DevOps Engineer para a decisão de configuração; Documentation Specialist para corrigir os comentários enganosos em `js/supabase-config.js` e `README.md` assim que a decisão (a) vs. (b) for tomada.
