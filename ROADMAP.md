...
- ✅ **RFC-036 Fase A — camada 3D decorativa "Correnteza do Conhecimento" atrás da trilha Aprender/Empreender** (v1.60.0): cena Three.js de fundo (oceano, gemas, anéis, bolhas, plâncton) animada por scroll via GSAP ScrollTrigger, aditiva e sem nenhum acoplamento com progresso/XP/quiz da trilha (RFC-035, confirmado por QA). Nova dependência via CDN (GSAP 3.15.0 + ScrollTrigger 3.15.0, versão travada + SRI, mesmo padrão de RFC-027). Um achado do QA (gemas/anéis nascendo sobre o cabeçalho em mobile estreito) foi corrigido e reverificado (110 amostras, 0 violações) antes do fechamento da fase. Pendência não bloqueante escalada ao Product Owner: falta ambiente de staging Supabase para validar o fluxo de lição completo com conta real de ponta a ponta. **Fase A.3 — riqueza crescente conforme o avanço na rolagem, concluída** (v1.63.0): a mesma cena (`js/trailscene3d.js`, sem módulo novo) ganhou 4 gemas douradas extras, 3 baús do tesouro (`THREE.Group`) e clusters de 18 moedas/14 pérolas (`InstancedMesh`), com densidade crescendo suavemente (`smoothstep`, sem "pop-in") conforme a mesma métrica de scroll que já move o "mergulho" — decisão de produto confirmada de reagir à rolagem, não ao progresso real do jogador, preservando o zero acoplamento com `Trail.*`/`Business.*`/`Learn.*`/`STORAGE_KEYS`. Pool fixo de objetos (nunca criados/destruídos durante o scroll), orçamento de 31 unidades/39 objetos reais (teto 34), validado com o avatar navegável da RFC-037 rodando simultaneamente (dois contextos WebGL ativos ao mesmo tempo, cenário nunca testado antes desta fase) — custo de execução por frame medido em 0,35-1,2ms, margem de ~8-13x sobre o orçamento de 60fps. `prefers-reduced-motion` passou a mostrar um estado intermediário da riqueza no frame estático em vez de escondê-la. Aprovado pelo QA Engineer com 2 achados não bloqueantes, ambos de metodologia de teste (divergência de medição de FPS em ambiente headless sem vsync real; nota de harness sobre a classe `.visible`), não bugs de produto. Pendências não bloqueantes que seguem em aberto: validação em hardware móvel físico real e teste de "pior caso" do bioma `correnteza-dourada` (mesma cor `--gold` da composição de riqueza) com conta/curso reais. A Fase B desta mesma RFC (motor 3D navegável da Cidade Financeira) ainda não foi iniciada — ver entrada correspondente em "Cidade Financeira" abaixo.
- ✅ **RFC-037 — trilha "turbinada": celebrações maiores, temas por bioma e avatar navegável (concluída, 3 fases)** (v1.61.0 + v1.62.0): pedido direto do usuário para elevar o "game feel" da tela da trilha em 3 frentes, faseadas por risco/esforço pelo Software Architect. **Fase 1+2** (v1.61.0) — celebrações maiores (`js/trailmilestones.js` + `Fx.milestoneCelebration()`): "nível completo" e "a cada 5 lições cumulativas" disparam um card de tela cheia, 100% cosmético (nunca XP/moeda extra, confirmado por spy em runtime + auditoria estática) e coordenado com `js/achievements.js` (`suppressNotifyOnce()`) para nunca empilhar 2 notificações do mesmo marco; temas por bioma (`js/trailthemes.js`): 5 biomas oceânicos mapeados aos 18 níveis reais das 3 trilhas, aplicados via atributo `data-bioma` e resolvidos em CSS puro, estendendo a mesma metáfora de oceano da cena decorativa da RFC-036. Ambos aplicados desde o MVP às duas trilhas da aba Aprender (financeira/história unificada e Empreender), decisão do Product Owner. `js/trail.js`/`js/business.js` — só a linha do atributo `data-bioma` foi tocada; nenhuma função de progresso/desbloqueio foi alterada (confirmado por `git diff` + 2 QAs). Aprovado pelo QA Engineer sem ressalvas bloqueantes — 1 achado não bloqueante (sequência de 3 telas quando uma lição fecha simultaneamente uma história interativa e um múltiplo de 5 lições) encaminhado para avaliação futura. **Fase 3** (v1.62.0, final desta RFC) — avatar navegável 3D do PolvIn (`js/trailavatar3d.js`, novo; `js/polvinrig3d.js`, novo — rig do PolvIn extraído de `js/citypolvin3d.js` para um builder compartilhado, API pública da Cidade Financeira 100% preservada e regressão confirmada por 2 QAs) andando sobre o nó atual/último concluído da trilha financeira/história (Empreender fora deste MVP), posicionado 100% via leitura de DOM (`getBoundingClientRect()`) — zero acoplamento a `Trail.*`/`Business.*`, zero `STORAGE_KEYS` novo. Um bug de gravidade média (avatar colidindo com o rótulo de XP do nó anterior em mobile, ~40% das transições) foi encontrado pelo QA, corrigido pelo Frontend Engineer (compressão vertical em runtime) e reverificado sem ressalvas (0/60 sobreposições, eram 22/60 antes). ~42 meshes combinados (avatar + cena decorativa da RFC-036) em 2 contextos WebGL simultâneos, ~60fps confirmado. Nenhuma dependência de CDN nova em nenhuma das 3 fases.
...
- **RFC-036 Fase B — substituir o motor 2D (Phaser) por um jogo 3D navegável, ainda não iniciada.** Câmera livre/navegável em 3D no lugar do mapa 2D com Phaser; `js/citylife.js` (estado e regras de negócio: ciclo semanal, aposentadoria, negócios, imóveis) deve ser preservado integralmente — só a camada de apresentação (`js/citygame.js`/`js/citypolvin3d.js`) muda. Decisões já registradas (Software Architect e Gamification Designer, RFC-036 seções 2 e 4): MVP deve ser explicitamente menor que "portar tudo de uma vez" (primeiro só o cenário/chão navegável), preservando as 5 construções e os contratos de evento (`citylife:scenario`, `citylife:aposentadoria`) já existentes. Fase A desta mesma RFC (camada 3D decorativa na trilha Aprender) já concluída — ver "Identidade visual e motion design" acima.
